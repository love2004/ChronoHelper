...
class NetworkUtils:
    ...
    def __init__(self, logger, settings=None):
        ...
        self.lock = threading.Lock()  # 線程鎖，防止並發檢測
    ...
    def check_campus_network(self, verbose=True, check_second_hop=None):
        ...
        try:
            ...
            try:
                ...
                # 在背景線程中執行躍點檢測，避免阻塞UI
                threading.Thread(target=check_hop_thread, daemon=True).start()
            ...
            # 將check_in_progress設置為False的部分已移至線程內部或者提前返回的地方
            # 這裡不再需要，因為可能與線程操作產生競爭條件
